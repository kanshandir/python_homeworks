...
def io_func(logfile_path, result_file_path):
    ...
    with open(logfile_path, 'r') as read_txt:
        with open(result_file_path, 'w') as result_txt:
            ips = map(pure_func, read_txt.readlines())
            filtered_data = filter(lambda x: x is not None, ips)
            for i in filtered_data:
                result_txt.writelines(i + "\n")


def pure_func(file_line):
    if "304" in file_line.split():
...
def test_myfunc_negative():
    ...
    line = '218.30.103.62 - - [17/May/2015:11:05:17 +0000] "GET /projects/xdotool/xdotool.xhtml \
    HTTP/1.1" 404 - "-" "Sogou web spider/4.0(+http://www.sogou.com/docs/help/webmasters.htm#07)"'
    assert pure_func(line) is None


if __name__ == '__main__':
    io_func("apache_log.txt", "result.txt")